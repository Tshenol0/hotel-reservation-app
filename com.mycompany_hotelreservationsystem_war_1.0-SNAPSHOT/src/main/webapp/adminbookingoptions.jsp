        ...
        <main>
            ...
            <ul class="room-list scrollable-content">             
                <% if (available != null && available.size() > 0) { %>
                   ...
                   <li class="item">
                    <a href="RoomDetailServlet?rooms=<%=room.getId()%>&user=<%=user.getId()%>>" class="room">
                        <div class="div div1"><%= room.getType() %> <span>room</span></div>
                        <div class="div"><span>floor:</span> <%= room.getFloor() %></div>
                        <div class="div"><span>status:</span><%=room.getStatus()%></div>
                     
                    </a>
                    <% if (!buy.contains(room.getId())) { %>    
                        <a class="selection" href="AdminRoomSelectServlet?selection=select&room=<%=room.getId() %>&user=<%=user.getId()%>">select</a>
                        <% } else { %>
                        <a class="selection" href="AdminRoomSelectServlet?selection=unselect&room=<%=room.getId()%>&user=<%=user.getId()%>">unselect</a>
                        <% } %>                           
                   </li>
                   <%}}%>
                                 
                   <%if(buy.size()>0){%>
                        <a href="NewServlet?user=<%=user.getId()%>" class="book">book now</a>
                   <%}%>                   
            </ul>                 
            </div>
