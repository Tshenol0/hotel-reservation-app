<%-- 
    Document   : adminmakebooking
    Created on : 23 Jun 2024, 9:02:59 AM
    Author     : User
--%>
...
<html>
    ...
    <body>
        ...
        <main>
            <div class="add">
                <a href="AdminLoadRoomsServlet?user=<%=user.getId()%>">book</a>
            </div>
            ...
            <ul class="room-list scrollable-content">
                    <% if (bookings != null && !bookings.isEmpty()) { %>
                        <% for (Booking booking : bookings) { %>
                            <li class="item">
                                <a href="BookingsServlet?bookings=<%= booking.getId() %>" class="room">
                                    <div class="div">
                                        <span>Check-in:</span> <%= booking.getStart().toString().substring(0, 16) %>
                                    </div>
                                    <div class="div">
                                        <span>Check-out:</span> <%= booking.getEnd().toString().substring(0, 16) %>
                                    </div>
                                    <div class="div">
                                        <span>Status:</span> <%= booking.getStatus() %>
                                    </div>
                                    
                                    <div class="state-change">
                                        <% if (booking.getStatus().equals("pending") || booking.getStatus().equals("accepted")) { %>
                                            <a class="selection" href="selection" href="BookingsServlet?bookings=cancel&booking=<%=booking.getId()%>">Cancel</a>
                                            <a class="checkout" href="EditServlet?bookings=edit&booking=<%=booking.getId()%>&user=<%=user.getId()%>">edit</a>
                                        <% } else if (booking.getStatus().equals("late")) { %>
                                            <a class="selection" href="BookingsServlet?bookings=cancel&booking=<%=booking.getId()%>">Cancel</a>
                                        <% } %>
                                    </div>
